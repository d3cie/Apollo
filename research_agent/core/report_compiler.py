...
class ReportCompiler:
    ...
    async def compile_report(self, topic: str, questions: List[Question]) -> ResearchReport:
        # Use Gemini for initial content analysis
        analysis_prompt = self._prepare_analysis_prompt(topic, questions)
        initial_analysis = await self.gemini_model.generate_content_async(analysis_prompt)
